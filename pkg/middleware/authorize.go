...
func Authorize(db *gorm.DB, inputRole ...models.RoleId ) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if tokenStr == "" {
			r := utils.BuildErrorResponse(http.StatusUnauthorized, "error", "Token could not be found!", "Unauthorized", nil)
		...
		if err != nil {
			r := utils.BuildErrorResponse(http.StatusUnauthorized, "error", "Token is invalid!", "Unauthorized", nil)
			c.AbortWithStatusJSON(http.StatusUnauthorized, r)
			return
		}
		...
		if !ok {
			c.AbortWithStatusJSON(http.StatusUnauthorized, utils.BuildErrorResponse(http.StatusUnauthorized, "error", "Token is invalid!", "Unauthorized", nil))
			return
		}
		...
		if code, err := access_token.GetByID(db); err != nil {
			c.AbortWithStatusJSON(code, utils.BuildErrorResponse(http.StatusUnauthorized, "error", "Token is invalid!", "Unauthorized", nil))
			return
		}
		...
		if access_token.LoginAccessToken != tokenStr || userID != access_token.OwnerID || !access_token.IsLive {
			c.AbortWithStatusJSON(http.StatusUnauthorized, utils.BuildErrorResponse(http.StatusUnauthorized, "error", "Session is invalid!", "Unauthorized", nil))
			return
		}
		...
		if !authorizedRole && len(inputRole) > 0 {
			c.AbortWithStatusJSON(http.StatusUnauthorized, utils.BuildErrorResponse(http.StatusUnauthorized, "error", "role not authorized!", "Unauthorized", nil))
			return
		}
		...
		if !ok && !authoriseStatus {
			c.AbortWithStatusJSON(http.StatusUnauthorized, utils.BuildErrorResponse(http.StatusUnauthorized, "error", "status not authorized!", "Unauthorized", nil))
			return
		}
...
func GetIdFromToken(c *gin.Context) (string, interface{}) {
	...
	if !ok {
		return "", utils.BuildErrorResponse(http.StatusForbidden, "error", "Forbidden", "Unauthorized", nil)
	}
	return id, ""
}